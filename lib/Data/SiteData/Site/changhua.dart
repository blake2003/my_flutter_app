...
class HomePage18 extends StatelessWidget {
  static const String routeName = '/changhua';
  const HomePage18({super.key});

  Color _getColor(double c) => AppColors.getAqiColor(c);

  Future<void> _fetchData(BuildContext context) async {
    final log = AppLogger('changhua');
    try {
      log.i('開始獲取彰化數據');
      await context.read<MultiSiteProvider>().fetchAll(['彰化']);
      log.i('彰化數據獲取成功');
    } catch (e) {
      log.e('彰化未加載成功', e);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final records = provider.recordsOf('彰化') ?? [];

    log.i('獲取到彰化紀錄数: ${records.length}');
