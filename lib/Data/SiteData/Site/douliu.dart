...
class HomePage16 extends StatelessWidget {
  static const String routeName = '/douliu';
  const HomePage16({super.key});

  Color _getColor(double c) => AppColors.getAqiColor(c);

  Future<void> _fetchData(BuildContext context) async {
    final log = AppLogger('douliu');
    try {
      log.i('開始獲取斗六數據');
      await context.read<MultiSiteProvider>().fetchAll(['斗六']);
      log.i('斗六數據獲取成功');
    } catch (e) {
      log.e('斗六未加載成功', e);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final records = provider.recordsOf('斗六') ?? [];

    log.i('獲取到斗六紀錄数: ${records.length}');
