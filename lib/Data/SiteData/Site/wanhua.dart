...
class HomePage27 extends StatelessWidget {
  static const String routeName = '/wanhua';
  const HomePage27({super.key});

  Color _getColor(double c) => AppColors.getAqiColor(c);

  Future<void> _fetchData(BuildContext context) async {
    final log = AppLogger('wanhua');
    try {
      log.i('開始獲取萬華數據');
      await context.read<MultiSiteProvider>().fetchAll(['萬華']);
      log.i('萬華數據獲取成功');
    } catch (e) {
      log.e('萬華未加載成功', e);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final records = provider.recordsOf('萬華') ?? [];

    log.i('獲取到萬華紀錄数: ${records.length}');
