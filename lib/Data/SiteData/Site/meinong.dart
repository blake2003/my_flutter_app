...
class HomePage11 extends StatelessWidget {
  static const String routeName = '/meinong';
  const HomePage11({super.key});

  Color _getColor(double c) => AppColors.getAqiColor(c);

  Future<void> _fetchData(BuildContext context) async {
    final log = AppLogger('meinong');
    try {
      log.i('開始獲取美濃數據');
      await context.read<MultiSiteProvider>().fetchAll(['美濃']);
      log.i('美濃數據獲取成功');
    } catch (e) {
      log.e('美濃未加載成功', e);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final records = provider.recordsOf('美濃') ?? [];

    log.i('獲取到美濃紀錄数: ${records.length}');
