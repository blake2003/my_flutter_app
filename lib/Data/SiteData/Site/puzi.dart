...
class HomePage15 extends StatelessWidget {
  static const String routeName = '/puzi';
  const HomePage15({super.key});

  Color _getColor(double c) => AppColors.getAqiColor(c);

  Future<void> _fetchData(BuildContext context) async {
    final log = AppLogger('puzi');
    try {
      log.i('開始獲取朴子數據');
      await context.read<MultiSiteProvider>().fetchAll(['朴子']);
      log.i('朴子數據獲取成功');
    } catch (e) {
      log.e('朴子未加載成功', e);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final records = provider.recordsOf('朴子') ?? [];

    log.i('獲取到朴子紀錄数: ${records.length}');
