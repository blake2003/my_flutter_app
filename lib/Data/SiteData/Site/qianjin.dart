...
class HomePage10 extends StatelessWidget {
  static const String routeName = '/qianjin';
  const HomePage10({super.key});

  Color _getColor(double c) => AppColors.getAqiColor(c);

  Future<void> _fetchData(BuildContext context) async {
    final log = AppLogger('qianjin');
    try {
      log.i('開始獲取前金數據');
      await context.read<MultiSiteProvider>().fetchAll(['前金']);
      log.i('前金數據獲取成功');
    } catch (e) {
      log.e('前金未加載成功', e);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final records = provider.recordsOf('前金') ?? [];

    log.i('獲取到前金紀錄数: ${records.length}');
