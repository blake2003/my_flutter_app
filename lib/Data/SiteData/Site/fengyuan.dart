...
class HomePage20 extends StatelessWidget {
  static const String routeName = '/fengyuan';
  const HomePage20({super.key});

  Color _getColor(double c) => AppColors.getAqiColor(c);

  Future<void> _fetchData(BuildContext context) async {
    final log = AppLogger('fengyuan');
    try {
      log.i('開始獲取丰原數據');
      await context.read<MultiSiteProvider>().fetchAll(['豐原']);
      log.i('丰原數據獲取成功');
    } catch (e) {
      log.e('豐原未加載成功', e);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final records = provider.recordsOf('豐原') ?? [];

    log.i('獲取到丰原紀錄数: ${records.length}');
