...
class HomePage28 extends StatelessWidget {
  static const String routeName = '/shilin';
  const HomePage28({super.key});

  Color _getColor(double c) => AppColors.getAqiColor(c);

  Future<void> _fetchData(BuildContext context) async {
    final log = AppLogger('shilin');
    try {
      log.i('開始獲取士林數據');
      await context.read<MultiSiteProvider>().fetchAll(['士林']);
      log.i('士林數據獲取成功');
    } catch (e) {
      log.e('士林未加載成功', e);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final records = provider.recordsOf('士林') ?? [];

    log.i('獲取到士林紀錄数: ${records.length}');
