...
class HomePage30 extends StatelessWidget {
  static const String routeName = '/xizhi';
  const HomePage30({super.key});

  Color _getColor(double c) => AppColors.getAqiColor(c);

  Future<void> _fetchData(BuildContext context) async {
    final log = AppLogger('xizhi');
    try {
      log.i('開始獲取汐止數據');
      await context.read<MultiSiteProvider>().fetchAll(['汐止']);
      log.i('汐止數據獲取成功');
    } catch (e) {
      log.e('汐止未加載成功', e);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final records = provider.recordsOf('汐止') ?? [];

    log.i('獲取到汐止紀錄数: ${records.length}');
