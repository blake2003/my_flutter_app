...
class HomePage25 extends StatelessWidget {
  static const String routeName = '/pingzhen';
  const HomePage25({super.key});

  Color _getColor(double c) => AppColors.getAqiColor(c);

  Future<void> _fetchData(BuildContext context) async {
    final log = AppLogger('pingzhen');
    try {
      log.i('開始獲取平鎮數據');
      await context.read<MultiSiteProvider>().fetchAll(['平鎮']);
      log.i('平鎮數據獲取成功');
    } catch (e) {
      log.e('平鎮未加載成功', e);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final records = provider.recordsOf('平鎮') ?? [];

    log.i('獲取到平鎮紀錄数: ${records.length}');
