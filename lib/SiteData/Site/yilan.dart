...
class HomePage4 extends StatelessWidget {
  static const String routeName = '/yilan';
  ...
  Future<void> _fetchData(BuildContext context) async {
    final log = AppLogger('yilan');
    try {
      log.i('開始獲取宜蘭數據');
      await context.read<MultiSiteProvider>().fetchAll(['宜蘭']);
      log.i('宜蘭數據獲取成功');
    } catch (e) {
      log.e('宜蘭未加載成功', e);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final records = provider.recordsOf('宜蘭') ?? [];

    log.i('獲取到宜蘭紀錄数: ${records.length}');
